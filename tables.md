...
- **Static Data Mode**: Supports passing `customItems` to render the table with client-side data instead of fetching from GraphQL.
...
- **Compound Component Pattern**: Exports `ModelTableCompound` (as `ModelTable`) with sub-components:
  ...
  - `ModelTable.Header`
- **Standard Mode**: Exports `ModelTable.Standard` for a pre-configured, zero-setup table layout.
...
- **Field Chips**: Pin specific fields (e.g., Status, Type) as quick-access filter chips on the toolbar.
...
- **Performance**: Caches related value labels to minimize network requests.

### **Filtering UI**
- **Active Filter Display**: Chips showing all currently applied filters with "clear" options.
...
- **Change Visualization**: Detailed view of field-level changes (Old vs New values).
...
### **Event Logging**
- **Action Tracking**: Automatically logs user interactions (creations, updates, deletions) via `useAuditableAction`.
- **Error Tracking**: Captures and logs GraphQL or network errors via `useModelTelemetry`.
- **Permission Logging**: Tracks permission checks and denials for security auditing.
