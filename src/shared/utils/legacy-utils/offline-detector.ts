...
export const testServerConnectivity = async (
  ...
  console.warn(
    "Server connectivity test failed for all health endpoints:",
    HEALTH_CHECK_PATHS,
  );
  return false;
};
...
export const getOfflineMessage = (
  context: "login" | "general" | "protected" = "general",
): string => {
  switch (context) {
    case "login":
...
export const onOfflineStatusChange = (
  callback: (isOffline: boolean, context?: string) => void,
) => {
