...
export function normalizeTemplateType(template: TemplateInfo): TemplateActionType {
...
function resolveTemplateFormFieldType(rawType?: string | null): TemplateFormFieldType {
...
export function parseTemplateClientFields(template: TemplateInfo): TemplateClientField[] {
...
export function buildTemplateClientSchema(fields: TemplateClientField[]): FormSchema {
  return {
...
function buildTemplateRequestUrl(
  ...
  const resolved = normalizeTemplateApiPath(resolveTemplateUrl(template.endpoint ?? ""));
  if (!resolved) return "";
...
async function mergeExcelBlobs(
  blobs: Blob[],
  rowIds: string[],
): Promise<Blob> {
...
export async function executeTemplateForRows(
  ...
  const templateType = normalizeTemplateType(template);
  const safeBase = sanitizeFileBase(template.key || template.title || "template");

  if (orderedIds.length === 1) {
    const rowId = orderedIds[0];
    const { blob, response } = await fetchTemplateBlob(template, rowId, clientData);

    if (templateType === "pdf") {
      const blobUrl = window.URL.createObjectURL(blob);
      const opened = window.open(blobUrl, "_blank", "noopener,noreferrer");
      if (!opened) {
        downloadBlob(blob, `${safeBase}-${rowId}.pdf`);
      }
      window.setTimeout(() => {
        window.URL.revokeObjectURL(blobUrl);
      }, 60_000);
  ...
  if (templateType === "pdf") {
    ...
    downloadBlob(mergedResponse.blob, `${safeBase}-merged.pdf`);
