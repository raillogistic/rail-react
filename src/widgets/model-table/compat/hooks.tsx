...
export function mapTableMetadataToFilterSchema(
  ...
  const filterableFields: FilterableField[] = metadata.filterSchema.map(
    (field) => ({
      ...
      relationConfig:
        field.isNested && field.relatedModel
          ? {
              relatedApp: "",
  ...
  const presets: FilterPreset[] =
    metadata.filterConfig.presets?.map((p) => ({
      id: p.name,
      name: p.name,
  ...
  const relationFilters: RelationFilter[] = metadata.relationFilters.map(
    (rf) => {
      const relationRecord = rf as any;
      const relationName =
        relationRecord.relationName ??
        relationRecord.name ??
        relationRecord.fieldName ??
        "";
      return {
        name: relationName,
        fieldName: relationName,
        fieldLabel: relationName,
        relationType: rf.relationType as
          | "FOREIGN_KEY"
          | "MANY_TO_MANY"
          | "REVERSE_FK"
          | "ONE_TO_ONE",
 ...
 */
const normalizeOrderingInput = (ordering?: string | string[]): string[] => {
 ...
 */
const resolveOrderingWithDefault = (ordering?: string | string[]): string[] => {
...
export function useGraphQLModelTable({
  ...
  const wherePayload = useMemo(() => {
    const columnClauses = columnFilters
      .filter(
        (f: any) => f.value !== undefined && f.value !== null && f.value !== "",
      )
      .map((f: any) => {
