...
import {
 ...
 toGraphqlFieldName,
 toSnakeCase,
 type ResolvedBaseModelTableFieldsConfig,
} from "../utils";
...
export function buildColumnDefinitions(
 ...
 [field.name, field.fieldName, canonical, toSnakeCase(canonical), toCamelCase(canonical)]
 ...
 const syntheticSource = field ? getSyntheticRelationCountSource(field) : undefined;
 if (syntheticSource) {
 ...
 const relationConfig = relations?.[root] ?? relations?.[toSnakeCase(root)] ?? relations?.[toCamelCase(root)];
 ...
 (relationCountSource
 ? (_value, row) => {
 const relationValue = row[relationCountSource];
 return Array.isArray(relationValue) ? relationValue.length : 0;
 }
 : undefined),
 };
 };
