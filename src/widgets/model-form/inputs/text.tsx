...
const TextInput: React.FC<Props> = ({ config, field, form }) => {
 ...
 data-slot="textarea"
 className={cn(
 "min-h-[100px] w-full resize-y border-border/60 bg-background/50 px-4 py-3 text-sm transition-all focus:border-primary/50 focus:bg-background focus:ring-4 focus:ring-primary/5 focus-visible:ring-0",
 ...
 className="cursor-pointer file:mr-4 file:border-0 file:bg-primary/10 file:px-4 file:py-1 file:text-xs file:font-bold file:text-primary hover:file:bg-primary/20"
 onChange={(event) => {
 ...
 <div 
 key={`${file.name}-${idx}`}
 className="flex items-center gap-2 border border-border/50 bg-muted/30 px-2 py-1.5 text-[11px] font-medium"
 >
 ...
 return (
 <FieldWrapper config={config} fieldId={fieldId} error={error} dirty={dirty}>
 <div className="group/json relative overflow-hidden border border-border/60 transition-all focus-within:border-primary/50 focus-within:ring-4 focus-within:ring-primary/5">
 ...
 disabled={config.disabled}
 className={cn(
 "h-10 border-border/60 bg-background/50 px-4 transition-all focus:border-primary/50 focus:bg-background focus:ring-4 focus:ring-primary/5 focus-visible:ring-0",
