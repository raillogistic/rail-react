...
const ChoiceInput: React.FC<Props> = ({ config, field, form }) => {
 ...
 className={cn(
 "h-auto min-h-10 w-full justify-between border-border/60 bg-background/50 px-3 py-2 text-left font-normal transition-all hover:border-primary/50 hover:bg-background",
 ...
 key={opt.value} 
 variant="secondary" 
 className="bg-primary/10 text-primary border-none px-1.5 py-0 text-[10px] font-bold uppercase tracking-wider"
 >
 {opt.label}
 <X 
 ...
 key={option.value}
 className={cn(
 " px-3 py-2.5 transition-colors focus:bg-primary/5 focus:text-primary",
 ...
 key={option.value}
 className={cn(
 "group relative flex w-full cursor-pointer items-center gap-3 border border-border/40 bg-background/50 px-4 py-3 transition-all duration-200",
 ...
 {value === option.value && <div className="size-2 bg-primary-foreground" />}
 </div>
 <div className="flex flex-col gap-0.5">
 <span className={cn(
 ...
 id={field.name}
 data-slot="select-trigger"
 className="h-10 w-full border-border/60 bg-background/50 px-4 transition-all focus:border-primary/50 focus:bg-background focus:ring-4 focus:ring-primary/5 focus-visible:ring-0"
 >
 ...
 disabled={option.disabled}
 className="py-2.5 transition-colors focus:bg-primary/5 focus:text-primary"
 >
 <div className="flex flex-col gap-0.5">
