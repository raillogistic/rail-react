    ...
    return (
      <div className="space-y-6 pb-6">
        ...
        {Object.entries(filteredGroups).map(([key, groupPresets]) => {
          ...
          const title = key === "static" ? "Built-in Vews" : key === "saved" ? "My Views" : "Shared with me";
          const icon = key === "static" ? <Star className="h-4 w-4 text-yellow-500" /> : key === "saved" ? <Bookmark className="h-4 w-4 text-blue-500" /> : <Users className="h-4 w-4 text-green-500" />;
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {groupPresets.map((preset) => (
                  <PresetCard
                    key={preset.id}
                    preset={preset}
                    isSelected={selectedPresets.includes(preset.source === "static" ? preset.name : preset.id)}
        ...
        {totalCount > 0 && search && Object.values(filteredGroups).every(g => g.length === 0) && (
          <div className="text-center py-12">
            <p className="text-sm text-muted-foreground">No matching views found for "{search}"</p>
          </div>
        )}

        <AlertDialog open={!!deleteDialogPreset} onOpenChange={(open) => !open && setDeleteDialogPreset(null)}>
          <AlertDialogContent className="rounded-2xl">
            <AlertDialogHeader>
              <AlertDialogTitle>Delete saved view?</AlertDialogTitle>
              <AlertDialogDescription>
                Are you sure you want to delete "<strong>{deleteDialogPreset?.name}</strong>"? This action cannot be undone and you will lose this configuration.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            disabled={disabled}
          >
            <Bookmark className={cn("h-4 w-4", selectedCount > 0 ? "text-primary fill-primary/10" : "text-muted-foreground")} />
            <span className="text-xs font-semibold">{label}</span>
            {selectedCount > 0 && (
              <Badge variant="default" className="ml-1 h-5 min-w-5 px-1 flex items-center justify-center rounded-full text-[10px]">
                {selectedCount}
              </Badge>
            )}
        ...
        <PopoverContent className="w-[320px] p-0 rounded-xl border-border/50 shadow-2xl overflow-hidden" align="start">
          <Command>
            <CommandInput
            ...
            <CommandList className="max-h-[300px]">
              ...
              {filteredGroups.saved.length > 0 && (
                <>
                  <CommandSeparator />
                  <CommandGroup heading={<span className="text-[10px] font-bold uppercase tracking-tighter">My Favorites</span>}>
              ...
              {filteredGroups.shared.length > 0 && (
                <>
                  <CommandSeparator />
                  <CommandGroup heading={<span className="text-[10px] font-bold uppercase tracking-tighter">Shared Views</span>}>
            ...
            <div className="bg-muted/30 border-t p-2 px-3 text-[10px] text-muted-foreground font-medium flex items-center justify-between">
              <span>{totalCount} total views</span>
  ...
  return (
    ...
    >
      <CardHeader className="p-3 pb-1 flex flex-row items-start justify-between space-y-0">
        <div className="flex-1 min-w-0 pr-2">
          <div className="flex items-center gap-2 mb-1">
            ...
            {isSelected && <Badge className="h-4 px-1 text-[9px] bg-primary uppercase tracking-tighter">Active</Badge>}
          </div>
          {preset.description ? (
            <CardDescription className="text-[11px] line-clamp-1">{preset.description}</CardDescription>
          ) : (
            <span className="text-[10px] text-muted-foreground/40 italic">No description provided</span>
          )}
        </div>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="rounded-xl shadow-xl border-border/50 min-w-[140px]">
              ...
              <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onToggle(); }} className="text-xs font-medium">
                <Check className="mr-2 h-3.5 w-3.5" /> {isSelected ? "Deselect" : "Select"}
              </DropdownMenuItem>
              {onShare && (
      ...
      <CardFooter className="p-3 pt-2 flex items-center justify-between border-t bg-muted/10">
        ...
        <div className="flex items-center gap-1">
          {preset.source === "static" && <Badge variant="outline" className="text-[9px] px-1 h-4 bg-yellow-500/10 text-yellow-600 border-yellow-200">SYSTEM</Badge>}
          {preset.isShared && <Badge variant="outline" className="text-[9px] px-1 h-4 bg-green-500/10 text-green-600 border-green-200">SHARED</Badge>}
        </div>
      </CardFooter>
    </Card>
  );
    ...
    >
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <div className={cn(
          ...
          isSelected ? "bg-primary border-primary text-primary-foreground scale-110 shadow-sm" : "border-muted-foreground/30"
        )}>
          {isSelected && <Check className="h-2.5 w-2.5" />}
        </div>
        <div className="flex-1 min-w-0">
          <span className={cn("text-xs font-semibold truncate block", isSelected && "text-primary")}>{preset.name}</span>
          {preset.description && <p className="text-[10px] text-muted-foreground truncate opacity-70">{preset.description}</p>}
        </div>
      </div>
      {showActions && (onEdit || onDelete || onShare) && (
        <div className="flex items-center opacity-0 group-hover:opacity-100 transition-opacity">
           <Button variant="ghost" size="icon" className="h-6 w-6 rounded-md hover:bg-muted" onClick={(e) => { e.stopPropagation(); onEdit?.(); }}>
