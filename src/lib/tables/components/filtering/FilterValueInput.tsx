...
import { FilterFieldType, FilterOptionType } from "../../types";
...
export const FilterValueInput: React.FC<Props> = ({
  ...
  if (!option) {
    return <Input disabled placeholder="Sélectionnez un champ" />;
  }
  ...
  if (
    ...
    return (
      <div className="grid grid-cols-2 gap-2">
        <Input
          placeholder={effectivePlaceholder ?? "Début"}
  ...
  if (option.filter_type === "BooleanFilter") {
    ...
    return (
      ...
      >
        <SelectTrigger>
          <SelectValue
            placeholder={effectivePlaceholder ?? "Sélectionnez une valeur"}
          />
        </SelectTrigger>
        <SelectContent>
  ...
  if (option.filter_type === "MultipleChoiceFilter") {
    ...
    if (!availableChoices.length) {
      return (
        <Input
          placeholder={
            effectivePlaceholder ?? "Valeurs séparées par des virgules"
          }
          value={
...
const RelatedModelFilterInput: React.FC<RelatedModelFilterInputProps> = ({
  ...
  const displayLabel = React.useMemo(() => {
    if (selectedValues.length === 0) {
      return (
        placeholder ??
        (isMulti ? "Sélectionnez des valeurs" : "Choisir une valeur")
      );
    }
