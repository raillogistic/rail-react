...
describe("ModelTable V2 Migration", () => {
  ...
  describe("useModelTableMetadataV2", () => {
    it("should fetch and map V2 metadata successfully", async () => {
      const mockV2Response = {
        modelSchema: {
          app: "inventory",
          model: "Product",
      ...
      const { result } = renderHook(() =>
        useModelTableMetadataV2("inventory", "Product")
      );
      ...
      expect(result.current.metadata?.app).toBe("inventory");
    ...
    it("should skip query when skip option is true", () => {
      const { result } = renderHook(() =>
        useModelTableMetadataV2("inventory", "Product", { skip: true })
      );
    ...
    it("should refetch with network-only policy", async () => {
      const mockV2Response = {
        modelSchema: {
          app: "inventory",
          model: "Product",
      ...
      result.current.refetch().then((metadata) => {
        expect(metadata).toBeDefined();
        expect(metadata?.app).toBe("inventory");
      });
